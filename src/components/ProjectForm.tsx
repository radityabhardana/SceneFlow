...
export function ProjectForm({ project }: Props) {
  ...
  return <form action={action} className="space-y-7">
    ...
    <div className="grid gap-6 md:grid-cols-2">
      ...
      <Field label="Genre" name="genre" value={value('genre')} error={error('genre')} required placeholder="e.g. Mystery drama" />
      <Field label="Tone" name="tone" value={value('tone')} error={error('tone')} required placeholder="e.g. Quiet, uneasy" />
      <div>
        ...
        <select id="aspectRatio" name="aspectRatio" defaultValue={value('aspectRatio')} required className={fieldClass(Boolean(error('aspectRatio')))}>
    ...
    <Field label="Visual style" name="visualStyle" value={value('visualStyle')} error={error('visualStyle')} required multiline placeholder="Describe the visual language, palette, and camera feel." />
...
function Field({ label, name, value, error, multiline, required, placeholder }: { label: string; name: string; value: string; error?: string; multiline?: boolean; required?: boolean; placeholder?: string }) {
  return <div><label htmlFor={name} className="mb-2 block text-xs font-semibold uppercase tracking-[.14em] text-[#969b9a]">{label} {required && <span className="text-[#c5e86c]">*</span>}</label>{multiline ? <textarea id={name} name={name} defaultValue={value} placeholder={placeholder} rows={4} required={required} className={fieldClass(Boolean(error))} /> : <input id={name} name={name} defaultValue={value} placeholder={placeholder} required={required} className={fieldClass(Boolean(error))} />}{<ErrorText text={error} />}</div>;
}
